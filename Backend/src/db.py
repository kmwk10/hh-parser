...
def update_vacancies_table(text):
    for item in get_vacancy_data(text):
        try:
...
def update_resumes_table(text):
    for link in get_resume_links(text):
        try:
            res = get_resume(link)
            cur.execute("""
...
update_vacancies_table("python")

# print(cur.execute("SELECT * FROM vacancies;"))
